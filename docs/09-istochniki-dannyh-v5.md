...
**Статус:** v5 — каталог источников не менялся с v4; добавлена врезка «Что кормит детектор» и выправлены формулировки, где on-chain/funding/макро описывались как входы детектора (по итогам `01-detektor-rezhima-v5.md`, `22-…`)
**Дата проверки:** v1 — разведка; v2 — 2026-07-17, прогон коллекторов; v3 — 2026-07-18, Kraken; v4 — 2026-07-18, ревизия всех API; v5 — 2026-08-06, врезка + формулировки

> **Что кормит детектор.** Состояние режима определяется **исключительно ценой** (SMA200:
> `close > SMA200 → BULL`, иначе `BEAR`). On-chain, макро и ширина рынка — **модификаторы**:
> они влияют на размер позиции, но **не на состояние** (`01-detektor-rezhima-v5.md` §2).
> Отказ любого из этих источников не меняет состояние детектора — только снижает `confidence`
> (непроверенная эвристика) и, для соответствующей стратегии, размер позиции. Поэтому ниже
> «источник для C2/MVRV/funding/макро» читать как «источник модификатора», а не «вход детектора».

## Реализация в коде (этап 0) — обновление 2026-07-22

Догнали часть расхождений между этим доком и кодом. Раздел актуальнее статусов §5.

**Сделано и проверено вживую:**
- **Kraken-коллектор** (`KrakenCollector`, `--collect=kraken`, в расписании каждые 5 мин): часовой funding через **v4** `historicalfundingrates` — первый вызов бэкфиллит ~12 мес (проверено: 8981 ставок по `PF_XBTUSD`/`PF_ETHUSD`), таблица `kraken_funding`; снапшот `tickers` (mark, текущий и **предиктивный** funding, OI, vol24h) в `kraken_ticker`. Закрывает §5 №10 (funding+tickers) и предиктивный funding (#4) — для Kraken.
- **Импортёр bulk-архива OI** (`OiArchiveImporter`, `--backfill=oi-archive --symbols=BTCUSDT,ETHUSDT --from=2021-01-01`): суточные `metrics` → `open_interest` (exchange=`binance`), тот же PK, что у живого коллектора (идемпотентно), пропуск уже покрытых дней. Закрывает §4.1 на уровне реализации (проверено на 7 днях = 2016 точек).
- **CoinDesk RSS** URL исправлен на `/rss` (без слэша). Функционально и так работало — `ApiClient` следует за 308-редиректом.

**Осталось сделать:**
- Kraken `instruments` (спецификации: tickSize/contractSize/лотность — записка 10 §6.4, док. 12 §4в) и спот-OHLC (резерв) — **не реализованы**, низкий приоритет.
- **Прогнать полный** `oi-archive` 2021→ по BTC/ETH и топ-парам (пока только smoke-тест 7 дней).
- **Прогнать полный** backfill Kraken funding уже происходит автоматически при первом `--collect=kraken`; в проде — просто запустить.
- Funding из bulk-архива (§2.2) — **не нужен**: глубокий funding рынка уже снят с fapi (2019-09→), архивный маршрут избыточен.
- `bookDepth`-архив (частичная L2, §2.2/§4.5) — коллектора стакана нет; поздние этапы (док. 06/08).
- Предиктивный funding для **OKX/Binance** (#4) — реализован только для Kraken; добавить при необходимости.
- Прочее из §5(e–g): ключи CoinGecko/FRED/Coinglass, ретро-капитализации (§4.3), замена DefiLlama для S5 — по мере надобности.

### Ликвидации: источники (обновлено 2026-07-24)

Собираем forward-only с **трёх CEX** (WS, на Frankfurt-micro 24/7 → бэкап в B2):
- **Binance** `!forceOrder@arr` — **весь рынок**, объединённый UM+CM (поле `st`). Рабочий эндпоинт `wss://fstream.binance.com/market/stream` + SUBSCRIBE (устаревшие `/ws` и `/stream` отдают ack без данных — это была причина прежних нулей).
- **OKX** `liquidation-orders` (`instType=SWAP`) — **весь рынок** свопов.
- **Bybit** `allLiquidation.<symbol>` — пер-символьный (нет wildcard), поэтому подписываемся на **все ~758 linear-перпов** (список с REST `instruments-info` на каждом коннекте, батчи по 100; форс-реконнект раз в 6ч подхватывает новые листинги; флаг `liq-bybit-all-symbols`).

**Hyperliquid (DEX) — отложено.** Публичного **рыночного** фида ликвидаций нет: в общем `trades` метки нет, ликвидации доступны только пофильтрово через `userFills`/`userEvents` (по адресу, поле `liquidation`: `liquidatedUser/markPx/method`). Собрать все рыночные нельзя — при `method="market"` контрагент произвольный; максимум частично `method="backstop"` через мониторинг HLP-vault (шумно, смещённая выборка). Ради невосполнимых данных копить частичный/смещённый поток нецелесообразно (исказит корреляционный анализ). Вернуться, если Hyperliquid заведёт нормальный фид или будет конкретная нужда. Альтернатива для истории — Coinglass (агрегирует и DEX, платно/лимитно).
...
| Glassnode / CryptoQuant | полный C2 (модификатор `cycle_phase`) | платно | решение после валидации модификатора `cycle_phase` на MVRV-only (MVRV — вход модификатора, не детектора) |
