...
public final class MarketFillModel implements FillModel {
     ...
     * что перехват — единственное по-настоящему связывающее допущение модели, и
     * поверка по каждой заявке ({@link FillCheck}, 09.09.2026) показала, что он
     * же и единственный источник выдуманных сделок.
     *
     * <h2>Что измерено</h2>
     *
     * С выключенным перехватом ТОЧНОСТЬ 100% на всех шести парах: каждое
     * исполнение по нашей цене — настоящее. Но полнота падает до 14-41%.
     * С включённым полнота 76-91%, а точность держится там, где перехват в
     * меньшинстве, и разваливается там, где он правит:
     *
     * <pre>
     *   пара  доля перехвата   точность перехвата
     *   ADA   160 из 200       91%
     *   BTC   122 из 160       75%
     *   ETH    85 из  92       36%
     *   SOL    69 из  82       23%
     * </pre>
     *
     * То есть перехват НУЖЕН (без него модели нечем объяснить четыре пятых
     * сделок) и при этом ошибается — на SOL и ETH в двух случаях из трёх.
     * Это и есть главная незакрытая неточность модели.
     *
     * <h2>Что пробовали и отвергли</h2>
     *
     * ⚠️ Требовать ПОДТВЕРЖДЕНИЯ СВИПА КНИГОЙ (настоящий агрессор, продавший
     * ниже нашего бида, обязан был съесть всё, что стояло выше, — значит
     * следующий снимок покажет лучший бид не выше цены принта). Проверено
     * 09.09.2026: правило делает ХУЖЕ на всех шести парах. Полнота падает
     * (BTC 84.9→79.6, PEPE 87.1→64.5, ADA 84.9→76.6), точность не растёт
     * (BTC 80.6→79.6, SOL 35.4→31.2). Причина в том, что снимок книги идёт раз
     * в 1.5 с, и к следующему снимку книга уже пополнена — проверка режет
