...
public final class DriftPersistence {
    ...
    public record Stats(double slope, double correlation, double rSquared,
         ...
         */
        /**
         * Сколько в выборке НЕЗАВИСИМЫХ наблюдений.
         *
         * Точки берутся с шагом порядка минуты, а горизонт — часы, поэтому
         * соседние наблюдения перекрываются десятками крат. Считать значимость
         * по {@code points} значит завысить её в {@code √overlap} раз: 7050 точек
         * при перекрытии ×120 — это 59 наблюдений, а не семь тысяч.
         */
        public double effectivePoints() {
            return overlap > 0 ? points / overlap : points;
        }

        /**
         * Значимость IC в сигмах на НЕЗАВИСИМОЙ выборке: {@code IC · √N_эфф}.
         *
         * Именно эта величина, а не сам IC, отвечает на вопрос «направление
         * дрейфа отличимо от нуля». На нашем окне выходит 0.66σ там, где по
         * сырому числу точек получилось бы 7.1σ (док. 148 §5).
         */
        public double tStat() {
            double n = effectivePoints();
            return n > 1 ? correlation * Math.sqrt(n) : 0;
        }

        /**
         * Отличимо ли направление дрейфа от нуля.
         *
         * Порог 2σ на НЕЗАВИСИМОЙ выборке. Прежняя версия проверяла
         * {@code |IC| ≥ 0.05} при {@code points ≥ 100} — то есть не проверяла
         * значимость вовсе и объявляла «направленное содержание» на оценке в
         * две трети сигмы (док. 148 §5).
         */
        public boolean predictive() {
            return Math.abs(tStat()) >= 2 && effectivePoints() >= 20;
        }
    }
