...
public final class ExecutionModel {
    ...
    private static final class Resting {
        ...
        boolean improving;
        boolean visible;
        /** Простояла ли заявка тик по той же цене — тогда очередь не пересчитывается. */
        boolean priceUnchangedSinceTick;
    ...
    public void place(Side side, double price, double qty, BookView book, long tsMs) {
        ...
        // Новая заявка приоритета не наследует: она только что встала в очередь.
        order.priceUnchangedSinceTick = false;
    ...
    private void applyVisibility(Resting order, BookView book) {
        ...
        // ⚠️ ПРИОРИТЕТ ОЧЕРЕДИ. Раньше эта строка выполнялась на КАЖДОМ refresh(),
        // то есть каждый тик, и перезаписывала накопленный приоритет свежим
        // снимком. Следствие было шире, чем потерянный механизм M2: модель
        // трактовала любую заявку как только что выставленную, а значит
        // **перевыставление в ней ничего не стоило**. Это систематически льстило
        // частому перевыставлению и штрафовало липкость — ровно ту ось, по которой
        // выносился приговор в док. 110 (найдено в разборе, док. 123 §6).
        //
        // Теперь заявка, простоявшая тик по ТОЙ ЖЕ цене, сохраняет очередь.
        // Вычитать прошедший объём отдельно не нужно: очередь уже укорачивается
        // им в onWindow — достаточно её НЕ ПЕРЕЗАПИСЫВАТЬ.
        //
        // ⚠️ НО ИЗМЕРЕНО (док. 125 §2): на наших данных правка НИЧЕГО НЕ МЕНЯЕТ —
        // все 42 прогона совпали до последнего знака, включая контроль M2, где
        // сброс очереди и её сохранение теперь разные ветки кода. Причина в книге:
        // снимок стенда — ПЯТЬ уровней, и на цене нашей заявки размера нет в
        // 25 683 074 случаях из 25 690 444 (99.97%). Очередь в модели принимает
        // почти только значения 0 (мы улучшаем книгу) и ∞ (мы вне видимой части);
        // конечной очереди, которая могла бы рассасываться, просто не возникает.
        //
        // Отсюда: заявленное в док. 123 §6 систематическое смещение ВСЕХ прежних
        // измерений в пользу частого перевыставления — НЕ ПОДТВЕРДИЛОСЬ. Канал
        // смещения в коде был, сигнала в нём нет. Мерить приоритет очереди этим
        // приближением нельзя в принципе: нужна книга глубже пяти уровней.
        // Правка оставлена как корректная — она перестаёт врать, когда данные
        // станут глубже, — но выводов на ней строить нельзя.
        if (improves) {
            order.queueAhead = 0;
        } else if (!order.priceUnchangedSinceTick) {
            order.queueAhead = book.qtyAt(order.side, order.price, limits.priceTolerance());
        }
        order.priceUnchangedSinceTick = true;
