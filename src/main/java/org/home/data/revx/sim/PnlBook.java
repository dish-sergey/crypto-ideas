...
public final class PnlBook {
    ...
    /**
     * P&L, если бы цена ВЕРНУЛАСЬ к значению на входе в окно.
     *
     * Нужна потому, что обычное разложение оценивает инвентарь по цене на КОНЦЕ
     * окна, а окно падения кончается на дне. Вопрос «мы правда проигрываем или
     * просто меряем в худшей точке» иначе не отделить: { total} и
     * { buyAndHold} на таком окне оба отражают одну и ту же нереализованную
     * переоценку, и их разность — не эдж, а разница НАКОПЛЕННОЙ позиции.
     *
     * Контроль buy and hold в этой точке тождественно равен нулю: цена вернулась,
     * держать было не на чем заработать. Поэтому это число само по себе и есть
     * альфа сценария возврата.
     */
    public double markAtStart(double fairFirst) {
        return cash + inventory * fairFirst - fees;
    }
