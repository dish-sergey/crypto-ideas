...
public class Executor {
    ...
    public void run() {
        ...
        // Шаг цены нужен циклу только для парковки при выключении: отведённая
        // заявка обязана лечь на допустимый тик, иначе замена не пройдёт и
        // заявку придётся снимать — то есть платить постановкой при возврате.
        loop.quoteStep(spec.quoteStep());
    ...
    public void pairSweep(String from, String to, String offsets, int levels,
        ...
        org.home.data.revx.replay.PairSweep.run(standDbPath, cfg, from, to,
                levels, levelStepBp, innerFirst, off, only, lotsUsd, thin, dynK);
    }
