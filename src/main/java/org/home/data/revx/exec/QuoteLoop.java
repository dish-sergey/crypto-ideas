        ...
        if (!quoting.get()) {
            ...
            for (ActiveOrder order : orders) {
                if (isParked(order.price())) {
                    log.info("заявка {} по {} припаркована — оставляю до запуска",
                            order.id(), fmt(order.price()));
                    continue;
                }
                cancelStray(order, why);
            }
            // Слоты обнуляются в любом случае: припаркованные заявки вернутся
            // через усыновление, когда котирование включат, и займут слоты по
            // цене — так же, как после любого другого перерыва.
