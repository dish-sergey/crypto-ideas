...
public final class FifoLedger {
    ...
    /**
     * ОБОРОТ чисто торговых пар в валюте котировки — честный знаменатель.
     *
     * ⚠️ Делить доход на ЧИСЛО ПАР можно, только пока все сделки одного размера.
     * Как только лот меняется, пара перестаёт быть единицей: продажа новым лотом
     * в $3 закрывает три старые партии по $1 и попадает в отчёт как ТРИ пары.
     * Доход тот же, «на сделку» падает втрое, и сравнивать настройки становится
     * нечем. Так и вышло 09.09.2026 при переходе с лота $1 на $3.
     *
     * Оборот от размера лота не зависит вовсе, поэтому «доход в базисных пунктах
     * оборота» сравним и между настройками, и между парами — этой же мерой
     * пользуется {@code --revx-exec-report}.
     */
    public double tradingClosedNotionalSince(long fromMs) {
        return sum(fromMs, r -> !r.handover(), r -> r.qty() * r.exit());
    }
