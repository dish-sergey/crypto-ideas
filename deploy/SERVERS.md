...
- **⚠️ A — ТОРГУЮЩИЙ контроль (с 02.09.2026, док. 128 §3).** Раньше A стоял на
  умолчаниях с целью инвентаря 0 и котировал двусторонне лишь **7.6% времени** —
  то есть был не контролем, а программой лимитных покупок, и ни пол, ни поводок
  бота B этим окном не проверялись вовсе. Теперь у обоих `skew-target=0.3`, а
  различие ровно в механизмах: у A пол и поводок выключены **явно**
  (`cost-floor-margin=-1`, `anchor-leash=-1`), у B включены. Разводить механизмы
  надо по БОТАМ, а не по окнам: развод по окнам смешивается с режимом.
  **Требует передеплоя юнита и повторного `/start`.**
- **⚠️ Ботов на аккаунте ТРИ (с 02.09.2026).** `ExecLimits.BOTS_SHARING_ACCOUNT = 3`,
  и суточный потолок постановок делится на всех: **266 на бота** при 1000 у
  площадки на весь аккаунт. Константа компилируется в jar, поэтому при
  добавлении бота **пересобирать и перевыкатывать надо ВСЕ три** — иначе старые
  считают себя вдвоём и втроём могут выбрать 1066 постановок.

  **Раскладка с 06.09.2026 — СЕТКИ на обеих парах, бот C остановлен.**

  | Бот | Пара | Конструкция | Лот | Постановок/сут | Юнит |
  |---|---|---|---|---|---|
  | A | BTC/USDC | **сетка 7/9/11 б.п.**, от дальнего | $1 | **400** | `revx-exec` |
  | B | SOL/USDC | **сетка 8/10/12 б.п.**, от дальнего | $1 | **300** | `revx-exec-b` |
  | C | — | **ОСТАНОВЛЕН** | — | 200 (резерв) | `revx-exec-c` |

  Сумма 900 из 1000. Четыреста у A не «на всякий случай»: сетке измерено 278
  постановок в сутки против 233 у одиночной котировки, а при исчерпании лимита
  бот не пропускает постановку, а **выключается совсем**.

  ⚠️ **Лот у B возвращён с $2 на $1.** Удвоение лота само по себе работало
  (+94% на BTC), но с сеткой несовместимо: три уровня с лотом $2 дают −0.0979
  по верхней границе, то есть убыток. Отношение потолка к лоту падает ниже семи
  ступеней, и бот подолгу стоит на нуле или в потолке.

  **Результат опыта с отступом (05.09, 10.1 ч, вне выборки).** Прогноз стенда
  был сделан ЗАРАНЕЕ: A получит 12 исполнений, C — 71 за 17 часов.

  | | Предсказано (17 ч) | Факт (10.1 ч) | В пересчёте |
  |---|---|---|---|
  | A (10 б.п.) | 12 | 10 | ~17 |
  | C (7 б.п.) | 71 | 95 | ~160 |

  Направление подтвердилось: C забрал почти весь поток. Соотношение
  предсказано 1:5.9, вышло 1:9.5 — эффект отступа сильнее прогноза. По
  величине стенд занизил обоих, C вдвое с лишним. Постановки C — 232 в сутки
  против предсказанных 233.

  ⚠️ Раскладка лимитов перевёрнута: расход идёт за исполнениями, а их больше
  у самого узкого отступа. Сумма 300 + 200 + 400 = 900 из 1000: сотня остаётся
  на повторы и ручные проверки, без неё зонд `--revx-order-probe` вышел бы за потолок.

  **A ↔ B меряет механизмы, A ↔ C меряет `κ`**: отношение частот исполнения на
  двух отступах даёт `κ = ln(λ_A/λ_C) / (d_C − d_A)` — величину, которая задаёт
  `δ*` и весь выбор пар, но живьём не мерилась ни разу (док. 132 §5).
- **⚠️ На счету $50 USDC и ноль BTC.** Потолок инвентаря у каждого бота
  0.00025 BTC ≈ $19.5, то есть втроём они могут запросить больше, чем на счету.
  Это не авария: `affordable` считает по `available`, и бот просто не поставит
  заявку. Но за долей отказов `POST` при трёх ботах надо следить
  (`--revx-exec-report` печатает её по каждому журналу).
- **Журнал:** `~/revx-exec/state/exec.db` — каждый запрос и ответ (ТЗ §6).
- **Отчёт по журналу:** `--revx-exec-report --journal=<путь> --out=<файл>` —
  разложение `total` на захват и инвентарную ногу плюс пошлина
  `c = отступ − захват − markout` (док. 128 §1). Считать пошлину как
  «`total` − захват» **нельзя**: это тождественно инвентарная нога.
  Журнал вытаскивается с ARM тонким срезом (без тела `exec_request`):
  `sqlite3 exec.db "ATTACH '/tmp/ea.db' AS o; CREATE TABLE o.exec_fill AS SELECT * FROM exec_fill; ..."`.
