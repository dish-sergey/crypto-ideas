...
- **⚠️ Ботов на аккаунте ТРИ (с 02.09.2026).** `ExecLimits.BOTS_SHARING_ACCOUNT = 3`,
  ...
  | Бот | Отступ | Цель | Пол | Поводок | Журнал | Юнит |
  |---|---|---|---|---|---|---|
  | A | 10 б.п. | 0.3 | нет | нет | `state/exec.db` | `revx-exec` |
  | B | 10 б.п. | 0.3 | **0** | **20 б.п.** | `state/exec-b.db` | `revx-exec-b` |
  | C | **14 б.п.** | 0.3 | нет | нет | `state/exec-c.db` | `revx-exec-c` |
