...
| **micro2** (сбор хвоста) | `instance-20260819-2319` | Oracle Always Free AMD (E2.1.Micro), 2 vCPU / 1 ГБ, 45 ГБ, Ubuntu 20.04 | eu-frankfurt-1 | `92.5.1.233` | `ssh -i D:\servers\oracle\instance-20260819-2319\ssh-key-2026-08-19.key ubuntu@92.5.1.233` |
...
### 6-бис. ШЕСТЬ ИСПОЛНИТЕЛЕЙ: боевые и опытные (с 11.09.2026)

На счёте работают ШЕСТЬ ботов двумя тройками. У каждого свой env-файл со своим
телеграм-токеном (`/etc/revx-exec*.env`) — иначе два потребителя `getUpdates`
дают 409 и молча отбирают управление друг у друга.

| бот | пара | отступ | цель скоса | лот | потолок | предел убытка |
|---|---|---|---|---|---|---|
| a | BTC/USDC | 12 б.п. | 30% |  |  |  |
| b | SOL/USDC | 18 б.п. | 30% |  |  |  |
| c | ETH/USDC | 14 б.п. | 30% |  |  |  |
| d | ETH/USDC | 8 б.п. | **50%** |  |  | **** |
| e | BTC/USDC | 6 б.п. | **50%** |  |  | **** |
| f | SOL/USDC | 10 б.п. | **50%** |  |  | **** |

**Первая тройка — боевая.** Ступени выбраны по сводному критерию: среднее
«доход / разброс по суткам» на растущем (24.08–06.09) и падающем (28.08–02.09)
окнах. 11.09 у b изменён отступ 21 → 18, у c 18 → 14; a не трогали.

**Вторая тройка — опыт**, проверяет связку «тесная ступень + большой скос»,
выведенную из сеток 11.09.2026, но никогда не наблюдавшуюся живьём. ⚠️ По модели
она у SOL и ETH СЛАБЕЕ боевой на падающем окне (на 15–25%); ставилась ради
проверки модели, а не дохода. Потолок втрое меньше, поэтому цена опыта мала.

⚠️ **Предел убытка абсолютный** (`ExecLimits.MAX_TRADING_LOSS_USDC = 1.0`), то
есть у мелкого бота он жёстче: доллар при потолке  это 5% капитала, при  —
14%. Поэтому у опытных он поднят до  через `--revx.exec.max-loss`; у боевых
параметр не задан и действует прежняя единица.

⚠️ **Боты d, e и f раньше торговали ADA, ENA и PEPE.** За `d` остаётся
открытая продажа ADA на 0.22254 (безубыточная, на 7% выше рынка) и захват
ADA 59.775 в реестре — владелец о ней знает и оставил намеренно. После смены
символа отвод при остановке (`ExecStopPost`) у d нацелен на ETH, то есть
ADA-заявку никто не снимет; так и задумано.

⚠️ **Список наблюдаемых ботов у `revx-info` ведётся РУКАМИ** (`revx.info.bots`)
и смену пары не переживает: 11.09 инфо-бот сутки показывал бы ADA/ENA/PEPE по
старым журналам — цифры правдоподобные и неверные. Меняешь пару в юните
исполнителя — меняй и там.

**Ведро постановок:** шесть ботов дают ~350 постановок в сутки из 1000.

### 7-бис. `revx-prune` (cron на micro) — ЧИСТКА базы стенда на ARM
- **Что:** `45 4 * * * /home/ubuntu/revx-prune.sh >> ~/revx-prune.log` — сносит с ARM
  строки книги и ленты старше `RETAIN_DAYS` (по умолчанию **21 сутки**). Скрипт в
  репо: `deploy/revx-prune.sh`. Появился 10.09.2026, когда книга по трём
  торгуемым парам стала собираться на 20 уровней: рост базы поднялся с ~250 до
  ~525 МБ/сутки, а на диске ARM 19 ГБ — без чистки чуть больше месяца до отказа
  записи, то есть до дыры в данных навсегда.
- **🔑 ЗАЩИТА — КУРСОР ВЫГРУЗКИ, А НЕ ВОЗРАСТ.** Удаляется только то, что уже
  уехало в инкремент: `rowid <= курсор` из `backup-cursor.txt`, и лишь ВТОРЫМ
  условием возраст. Если выгрузка неделю не работала, чистка не тронет ничего —
  просто удалит ноль строк. Возраст без курсора снёс бы данные, которых больше
  нигде нет.
- **⚠️ VACUUM НЕ ДЕЛАЕТСЯ, и это намеренно.** Файл не уменьшается, но
  освободившиеся страницы SQLite переиспользует под новые вставки — размер
  выходит на полку, а это и нужно. Полный VACUUM держит писателя и повторяет
  инцидент 25-26.08.2026 (см. ниже): 33 часа книги потеряно безвозвратно.
- **Порядок:** `15 3` выгрузка → `45 4` чистка. Полтора часа запаса.
- **Первый прогон 10.09.2026:** 624 307 строк книги + 15 784 сделки, 3 минуты,
  184 МБ свободных страниц, сбор не прерывался (`book_legs` рос всё время).
- **Проверить:** `tail ~/revx-prune.log` на micro · `df -h` на ARM. Вхолостую:
  `DRY_RUN=1 ~/revx-prune.sh` (только считает, ничего не удаляет).
- **Изменить срок:** `RETAIN_DAYS=30 ~/revx-prune.sh` либо правка cron. 21 сутки
  это ~11 ГБ на полке; 30 суток ~16 ГБ, а свободно 19 — впритык.

### 7-тер. Локальный архив — `scripts/revx-archive-pull.ps1`
- **Что:** забирает с micro инкременты, которых нет в `D:evx-datarchive`.
  Качает только отсутствующее, запускать можно сколько угодно часто.
- **⚠️ ЛОКАЛЬНАЯ КОПИЯ — ЕДИНСТВЕННАЯ ПОСТОЯННАЯ.** На micro инкременты живут
  60 суток, на ARM теперь 21. Всё, что старше двух месяцев, существует ровно
  на локальной машине. Раз в пару недель — обязательно.
- **Состояние на 10.09.2026:** 22 файла, 1.3 ГБ, включая полные копии
  `revx-2026081*..2026082*.db.gz` — база цепочки, удалять нельзя.
...
- **Глубина книги (с 10.09.2026):** пары, в которых котируем (`revx.fast-pairs` —
  BTC, ETH, SOL) собираются на **20 уровней** (`revx.book-depth-deep`), хвост
  остаётся на пяти (`revx.book-depth`). Уровни глубже пятого лежат в
  `revx_book.deep_bids` / `deep_asks` компактным текстом `цена:объём,…`; пять
  колонок `bp1..bp5` не тронуты, поэтому все прежние читатели работают как были.
  ⚠️ **Запросов это не добавляет** — `limit` параметр того же обращения, ведро
  лимитов не при чём (проверено: 857 ног за 2 минуты, 429 — ноль). Добавляет
  МЕСТА: ~530 байт на строку, около **275 МБ/сутки** сверх прежнего. На диске
  было 19 ГБ свободно, то есть примерно на два месяца — дальше нужна чистка
  старых суток или вынос архива.
  Потолок площадки — 100 уровней, `limit=200` отвечает 400. Замер охвата
  10.09.2026: пять уровней это 14-28 б.п. от середины, двадцать — 64-350 б.п.
  Проверить глубину: `--revx-depth-probe --symbol=BTC-USDC` (только чтение).
...
- **⚠️ Ботов на аккаунте ТРИ (с 02.09.2026).** `ExecLimits.BOTS_SHARING_ACCOUNT = 3`,
  ...
  **Раскладка с 09.09.2026 — ОДИН УРОВЕНЬ, лот $3, три пары.**

  | Бот | Пара | Конструкция | Лот | Потолок | Пост/сут (прогноз) | Юнит |
  |---|---|---|---|---|---|---|
  | A | BTC/USDC | **1 уровень, 12 б.п.** | $3 | $20 | 50 | `revx-exec` |
  | B | SOL/USDC | **1 уровень, 21 б.п.** | $3 | $20 | 40 | `revx-exec-b` |
  | C | ETH/USDC | **1 уровень, 18 б.п.** | $3 | $20 | 34 | `revx-exec-c` |
  | D, E, F | ADA, ENA, PEPE | **ОСТАНОВЛЕНЫ** | — | — | — | `revx-exec-d/e/f` |

  Прогноз спроса — **124 постановки в сутки** из тысячи против измеренных
  871 у шести ботов с трёхуровневой сеткой. Бюджет перестаёт быть узким
  местом.

  **Почему так** (замер 09.09.2026, 13 суток 24.08–06.09, `docs/pairs/`):
  один уровень против трёх при ОДИНАКОВОМ капитале. ADA, ENA и PEPE
  отставлены: у ADA нет запаса между спредом и волатильностью, у ENA и PEPE
  нет потока (30 и 48 сделок на ленте в сутки).

  ⚠️ **Обоснование ОСЛАБЛО после исправления модели** (09.09.2026, вечер).
  Довод был «у BTC три уровня не выиграли ни одного дня из тринадцати»; на
  стенде, который больше не позволяет двигать частично исполненную заявку,
  стало 4 победы из 13. Для BTC вывод тот же (7 побед из 13, лучший доход),
  **а для SOL одноуровневая форма — измеренно ХУДШАЯ: +3.57 против +4.03 у
  двух уровней (−12%)**. Бот B стоит на ней сознательно, ради простоты и
  экономии постановок, а не потому что так измерено. Там же уехал оптимум
  отступа SOL: 21 → 18 б.п. (+6%, на 13 сутках это шум).

  ⚠️ **Потолок к лоту = 6.7 ступени** при документированном пороге в семь
  (док. от 05.09.2026). Скос получает шесть-семь состояний вместо двадцати.
  Замеры сделаны с этим же потолком, то есть эффект в них учтён; ровно семь
  ступеней дал бы потолок $21.

  ⚠️ **Лот $3 требует jar с правкой частичных исполнений** (коммит 7368d1c).
  Под $3 попадает 11% принтов BTC, 4% SOL, 3% ETH — то есть частичные
  исполнения станут обычным делом, а до этой правки бот терял их МОЛЧА:
  узнать о сделке он может только по исчезнувшей заявке, а частично
  исполненная из книги не исчезает. Позиция бота начала бы расходиться с
  остатком счёта. При лоте $1 правка была не нужна — все 390 живых сделок
  за сутки были ровно в один лот.
...
## Сервисы на micro2 (92.5.1.233)

### `revx-tail` — сбор ХВОСТА вселенной Revolut X

- **Что:** `java -jar ~/revx/app.jar --revx-collect` с ограничением на 12 пар,
  не покрытых основным сбором: AVAX, BCH, BONK, DOGE, DOT, HBAR, JUP, SHIB,
  SUI, TON, TRUMP, TRX. Основной сбор на bot-arm держит остальные 11 из 23
  USDC-пар площадки.
- **Юнит:** `/etc/systemd/system/revx-tail.service` (копия — `deploy/revx-tail.service`)
- **БД:** `~/revx/data/revx.db`. **Рантайм:** `~/jre25` (скопирован с micro1 —
  ссылка на bell-sw для 25.0.4+11 отдавала не архив).
- **Проверить:** `systemctl status revx-tail` ·
  `sqlite3 "file:~/revx/data/revx.db?mode=ro" "SELECT symbol,COUNT(*) FROM revx_book GROUP BY symbol;"`

> **Почему отдельная машина, а не ещё один ярус на bot-arm.** Лимит публичного
> пути Revolut X — **по IP**, и ограничен там минимальный интервал между
> запросами (~1 с), а не средний темп. Второй адрес даёт вторую квоту целиком.
> Авторизованный путь так не размазать: там 1000/мин **на ключ**, а из этой
> тысячи уже платят сбор (551/мин) и шесть торговых ботов.

> ⚠️ **Ключей на micro2 нет и быть не должно.** Без них приложение молча
> остаётся на публичном пути — ровно то, что нужно. Положи сюда ключ, и сбор
> начнёт ходить в ту же тысячу в минуту, что и торговые боты, то есть смысл
> второй машины пропадёт целиком.

> ⚠️ **Бэкапа здесь НЕТ.** На micro1 данные льются в B2 через Litestream, тут
> нет ничего: хвост собирается «чтобы был», и потеря диска стоила бы только
> его. Понадобится история — либо поднять сюда Litestream, либо забирать базу
> так же, как базу стенда (`--revx-assemble` работает с любыми копиями).

Расклад нагрузки (12 пар = 24 ноги, X/USDC и X/USD для курса USDC/USD):
```
  книги   24 ноги / 60 с  = 24/мин = 0.40 req/s
  сделки  12 пар  / 900 с ≈  0.8/мин
  ИТОГО ≈ 0.41 req/s при потолке 0.8 и наблюдённом пределе ~1.0
```
