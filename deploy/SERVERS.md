...
- **⚠️ Ботов на аккаунте ТРИ (с 02.09.2026).** `ExecLimits.BOTS_SHARING_ACCOUNT = 3`,
  ...
  | B | **SOL/USDC** | **10 б.п.**, лот **$2** | **200** | `state/exec-b-sol.db` | `revx-exec-b` |
  | C | BTC/USDC | **7 б.п.** | **400** | `state/exec-c.db` | `revx-exec-c` |

  **A ↔ C — проверка лестницы отступов ВНЕ ВЫБОРКИ (с 05.09.2026).** Стенд на
  окне 04–05.09 дал максимум дохода на 7 б.п., а не на 10, и κ = 0.33…0.45 на
  б.п. Разница в деньгах там микроскопическая (семь десятых цента на лоте в
  доллар), окно было одно и оно же калибровочное — поэтому проверяем живьём.

  ⚠️ Прогноз стенда на эту пару настроек: C заберёт почти весь поток, у A
  исполнения просядут с 30 до 12 (по верхней границе с 36 до 16). То есть
  сравнение пойдёт «7 против 10 в конкуренции», а не в изоляции. Если живое
  разойдётся с этим — расходиться будет ИМЕННО с предсказанием, сделанным
  заранее, и это первый настоящий тест стенда на прогноз.
