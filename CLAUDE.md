...
- **⚠️ МОДЕЛЬ ИСПОЛНЕНИЯ СТЕНДА НЕ ОТКАЛИБРОВАНА (сверка 08.09.2026).** Первое
  прямое сличение живых ботов со стендом на одном окне (07.09 19:30 → 08.09
  13:11, шесть пар): живьём **337 исполнений**, модель дала **113**. Втрое
  меньше. После правки очереди (см. ниже) — 120 против 303 на трёх парах, то
  есть 2.4 раза. **Все числа стенда до этой сверки — оценки порядка, а не
  предсказания.**
  - лента и глубина ни при чём, это ПРОВЕРЕНО: 94–100% наших живых сделок
    находятся в собранных принтах, 94–100% исполнений случились внутри пяти
    видимых уровней. ⚠️ Сверять надо по времени ПРИНТА: отметка в `exec_fill` —
    это когда бот УЗНАЛ об исполнении, иногда на минуты позже. При окне ±3 с
    выходит «лента теряет половину сделок», при ±3 мин — 99%;
  - **очередь считалась один раз при постановке и не пересматривалась.** Она же
    исчезает не от торговли и не от отмен, а потому что ЦЕНА ПРИХОДИТ К НАМ.
    Измерено на 295 исполнениях: объём впереди при постановке против момента
    исполнения — ADA 3385 → 9.2 (в 368 раз), ETH 0.0104 → 0.0004. Исправлено:
    `MarketFillModel` берёт минимум с объёмом по ТЕКУЩЕЙ книге;
  - **убыль очереди от чужих отмен измерена, но она второстепенна.** Медианы
    BTC 0.119, ADA 0.172, ETH 0.271 доли в секунду (⚠️ смещены вверх: мерилось
    только по заявкам, которые исполнились). Калибровка дала НУЛЕВОЙ отклик —
    поправка уменьшала величину, которая и так не доходила до нуля. Параметр
    `-Drevx.queue.decay`, по умолчанию выключен;
  - **механизмы перепутаны местами.** Живьём мы зарабатываем ОЧЕРЕДЬЮ: у BTC
    351 принт прошёл по нашей цене или выше, взято 147 (42%), а перехват —
    редкость (BTC 13%, ADA 29%, ETH 9% принтов-продаж ниже нашего бида). В
    модели наоборот: «очередью 1, перехватом 102». Она получает похожий порядок
    величины НЕВЕРНЫМ способом, и потому расходится при смене условий. Это
    следующее, что чинить.
  - диагностика есть в отчёте прогона: «модель исполнения: очередью N,
    перехватом M, пропущено из-за невидимости K». До 08.09 счётчики считались,
    но никуда не выводились, и разложить расхождение было нечем.
