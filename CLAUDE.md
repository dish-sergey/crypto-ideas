...
- **⚠️ МОДЕЛЬ ИСПОЛНЕНИЯ СТЕНДА ЗАНИЖАЕТ СДЕЛКИ В 1.7 РАЗА (сверка 08.09.2026).**
  ...
  - ⚠️ **сопоставлять сделку с лентой надо с ДОПУСКОМ по цене.** Цена принта не
    равна цене нашей заявки: она лежит на доли базисного пункта ЗА ней (наш SELL
    79357.50 → принт 79357.60). При точном равенстве находится 19 сделок из 164,
    при допуске в 2 б.п. — 146. И сверять по времени ПРИНТА: отметка в
    `exec_fill` — это когда бот УЗНАЛ, иногда на минуты позже;
