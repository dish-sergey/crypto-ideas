...
- **⚠️ МОДЕЛЬ ИСПОЛНЕНИЯ СТЕНДА ЗАНИЖАЕТ СДЕЛКИ В 1.7 РАЗА (сверка 08.09.2026).**
  ...
  - **ПОВЕРКА ПО КАЖДОЙ ЗАЯВКЕ (`--revx-fill-check`, 09.09.2026) — правило
    исполнения ЛУЧШЕ, чем показывало сравнение числа сделок.** Прибор берёт все
    заявки, которые живой бот действительно поставил (цены, размеры, время
    жизни — из журнала запросов), и спрашивает по каждой: сказала бы модель, что
    она исполнится? Обратной связи нет, траектория не расходится, поэтому
    ошибка правила отделена от ошибки прогона. Окно 08.09 08:50 → 09.09 06:50,
    296 тыс. заявок:

    | пара | заявок | живьём | модель | полнота | точность |
    |---|---:|---:|---:|---:|---:|
    | ADA | 54 372 | 218 | 200 | 84.9% | **92.5%** |
    | BTC | 44 407 | 152 | 160 | 84.9% | 80.6% |
    | PEPE | 60 744 | 31 | 31 | 87.1% | 87.1% |
    | ENA | 39 424 | 11 | 18 | 81.8% | 50.0% |
    | ETH | 49 539 | 50 | 92 | 76.0% | **41.3%** |
    | SOL | 47 844 | 32 | 82 | 90.6% | **35.4%** |

    Запаздывание модели — медиана 0.0 с: когда она угадывает, она угадывает и
    момент. **Значит «модель занижает на 40%» относилось к СНОСУ ТРАЕКТОРИИ, а
    не к правилу:** в полном прогоне стенд ставит свои заявки, они исполняются
    иначе, расходится инвентарь, за ним скос и цены;
  - **весь ложный сигнал даёт ПЕРЕХВАТ.** С `-Drevx.fill.intercept=false`
    точность 100% на всех шести парах — исполнение по нашей цене всегда
    настоящее, — но полнота падает до 14-41%. Точность самого перехвата: ADA
    91%, BTC 75%, ETH 36%, SOL 23%. То есть допущение нужно (без него нечем
    объяснить четыре пятых сделок) и при этом на SOL и ETH ошибается в двух
    случаях из трёх. Это главная незакрытая неточность модели;
  - ⚠️ **подтверждение свипа книгой ПРОВЕРЕНО И ОТВЕРГНУТО.** Идея: настоящий
    агрессор, продавший ниже нашего бида, обязан был съесть всё выше, и
    следующий снимок это покажет. Делает хуже на всех шести парах — полнота
    падает (BTC 84.9→79.6, PEPE 87.1→64.5), точность не растёт (BTC 80.6→79.6).
    Снимок идёт раз в 1.5 с, книга к нему уже пополнена, и проверка режет
    настоящие свипы наравне с ложными;
